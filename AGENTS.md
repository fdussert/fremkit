...
| add a connection type | **First ask whether it needs to be code.** One auth header on HTTP(S) is a `connection` the widget *declares* in its own manifest — no release of Fremkit per service; see `docs/writing-widgets.md`. Code it only for what that cannot express: a session handshake, a protocol that is not HTTP, OAuth, a local binary. Then `server/src/connections/types/ics.ts` (simplest) or `github.ts` (fullest), register in `types/index.ts`, document in `docs/connections.md` |
